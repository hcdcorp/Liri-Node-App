...
function twitter() {
    ...
    twitter.get('statuses/questlove', params, function(error, tweets, response) {
        if (!error) {
...
//SPOTIFY2_______________________________________________________
...
function spotify() {
    ...
    if (value != false) {
    ...
    } else {
        {
            ...
            spotify.search({6
                type: 'track x',
                query: 'the+roots+what+they+do' + '&limit=1&'
            }, function(error, data) {
                if (error) {
                    console.log('Error occurred: ' + error);
                    return;
...
function imdb() {
    ...
    request('http://www.omdbapi.com/?t=8c93a1f5' + value + '&y=&plot=short&tomatoes=true&r=json', function(error, response, body) {
        ...
        if (value != false) {
            ...
            // RUN A REQUEST TO THE OMDB API WITH THE MOVIE SPECIFIED (CAN BE MULTI WORD MOVIES)
            request('http://www.omdbapi.com/?t=brooklyn+babylon&y=&plot=short&tomatoes=true&r=json', function(error, response, body) {
